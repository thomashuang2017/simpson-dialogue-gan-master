...
def disc_train_data(sess, gen_model, vocab, source_inputs, source_outputs,
    ...
    # train_query  加入已經補 pad 的 query
    # train_answer 加入已經補 pad 的 True answer
    # train_labels 加入 1 (represent True)
    ...
    def decoder(num_roll):
        for _ in xrange(num_roll):
            ...
            for seq in output_logits:
                ...
                # seq_tokens = 多句話 (一個 batch)
                
            # seq_tokens transpose
            seq_tokens_t = []
            ...
            # train_query  加入已經補 pad 的 query
            # train_answer 加入已經補 pad 的 Fake answer
            # # train_labels 加入 0 (represent Fake)
    ...
    if mc_search:
        train_query, train_answer, train_labels = decoder(gen_config.beam_size) # 生 7 個 Fake
    else:
...
def disc_step(sess, bucket_id, disc_model, train_query, train_answer, train_labels, forward_only=False):
    ...
    # softmax operation
    logits = np.transpose(softmax(np.transpose(logits))) # 轉成softmax
...
# Now you have a Generator based on Seq2Seq model with attention , inputs are your dataset ( True query and True answer)
# And you also have a Discriminator based on hierachy encoder with softmax_cross_entrophy and can classify the given inputs ( True query , True answer , False answer) 
# Here are implement the policy gradient
    
def al_train():
    
    """
    1.Sample (X,Y) from real disc_data
    2.Sample (X,Y) and (X, ^Y) through ^Y ~ G(*|X) is also just like hier_get_batch()
    3.Update D using (X, Y ) as positive examples and(X, ^Y) as negative examples
    4.Sample (X,Y) from real disc_data
    5.Sample (X,Y) and (X, ^Y) through ^Y ~ G(*|X) with Monte Carlo search
    6.Compute Reward r for (X, ^Y ) using D.---based on Monte Carlo search
    7.Update G on (X, ^Y ) using reward r
    8.Teacher-Forcing: Update G on (X, Y )
    ...
    with tf.Session() as sess:
        ...
        # All True query and True answer
        ...
        while True:
            ...
            if current_step % 200 == 0:
                ...
                for i in xrange(len(train_query)):
                    print("label: ", train_labels[i]) # label = 0 is Fake label = 1 is True
            ...
            # 這裡更前面的差別只在於 train_answer是用 monte carlo 生成的
            ...
            # The input is from Monte Carlo Search
